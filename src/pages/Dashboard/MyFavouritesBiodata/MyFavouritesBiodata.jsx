...
const MyFavouritesBiodata = () => {
  ...
  const [MyFavouritesBiodata, setMyFavouritesBiodata] = useState([]);
  const axiosSecure = useAxiosSecure();

  useEffect(() => {
    fetchMyFavouritesBiodata();
  }, []);

  const fetchMyFavouritesBiodata = async () => {
    try {
      const response = await axiosSecure.get("/checkouts");
      setMyFavouritesBiodata(response.data);
    } catch (error) {
  ...
  const handleDeleteRequest = async (id) => {
    try {
      ...
      if (result.status === 200) {
        ...
        setMyFavouritesBiodata(prevRequests => prevRequests.filter(request => request?._id !== id));
    ...
    <div className="container mx-auto px-4 ">
      ...
      <div className="overflow-x-auto">
        <table className="table-auto w-full border-collapse border border-gray-300">
          ...
          <tbody>
            {MyFavouritesBiodata.map((request) => (
              <tr key={request.id} className="border-b border-gray-300">
                ...
                <td className="px-4 py-2">{request.status === 'Approved' ? request?.email : 'Pending'}</td>
